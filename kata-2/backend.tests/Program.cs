...
internal static class TestHarnessProgram
{
    public static async Task<int> Main(string[] args)
    {
        ...
        var tests = new List<TestCase>
        {
            ...
            new("backend", "delete archives existing task", DeleteArchivesExistingTask),
    ...
    private static async Task CreateTrimsTitleAndDefaultsToPending()
    {
        ...
        AssertEqual("medium", result.Value.Priority, "Expected default priority.");
        AssertNull(result.Value.Description, "Expected description to default to null.");
    ...
    private static async Task ListRejectsInvalidStatus()
    {
        ...
        AssertEqual(ServiceErrorType.Validation, result.ErrorType, "Expected validation error.");
        AssertEqual(
            "Status must be pending, in_progress, completed, cancelled or archived.",
            result.ErrorMessage,
    ...
    private static async Task UpdateValidatesStatusAndTitle()
    {
        ...
        AssertEqual(ServiceErrorType.Validation, invalidPriority.ErrorType, "Expected invalid priority.");
        ...
        AssertEqual("Add notes", updated.Value.Description, "Expected updated description.");
        AssertEqual("high", updated.Value.Priority, "Expected updated priority.");
        AssertEqual("completed", updated.Value.Status, "Expected completed status.");
        await Task.CompletedTask;
    }

    private static async Task DeleteArchivesExistingTask()
    {
        var service = NewService();
        ...
        var archivedList = service.List("archived");
        var activeList = service.List(null);

        AssertTrue(deleteResult.IsSuccess, "Expected delete to succeed.");
        AssertTrue(getResult.IsSuccess, "Expected archived task to remain retrievable.");
        AssertEqual("archived", getResult.Value!.Status, "Expected archived status.");
        AssertNotNull(getResult.Value.ArchivedAt, "Expected archive timestamp.");
        AssertEqual(1, archivedList.Value!.Count, "Expected archived filter to return task.");
        AssertEqual(0, activeList.Value!.Count, "Expected default list to hide archived task.");
        await Task.CompletedTask;
    }
    ...
    private static async Task ControllerCreateReturnsCreatedResponse()
    {
        ...
        AssertEqual("medium", task.Priority, "Expected default priority.");
        await Task.CompletedTask;
    }
    ...
    private static async Task ControllerListRejectsInvalidStatus()
    {
        ...
        var result = controller.List("done");
        ...
        AssertEqual(
            "Status must be pending, in_progress, completed, cancelled or archived.",
            problem.Detail,
            "Expected controlled error detail.");
        await Task.CompletedTask;
    }
    ...
    private static async Task ControllerUpdateReturnsNotFoundForMissingTask()
    {
        ...
        var result = controller.Update(Guid.NewGuid(), new UpdateTaskRequest(null, null, null, "completed"));
    ...
    private static async Task ControllerUpdateRejectsEmptyPatchPayload()
    {
        ...
        var result = controller.Update(created.Id, new UpdateTaskRequest(null, null, null, null));
        var badRequest = AssertIsType<ObjectResult>(result.Result, "Expected ObjectResult.");
        var problem = AssertIsType<ProblemDetails>(badRequest.Value, "Expected ProblemDetails.");
    ...
    private static async Task ControllerUpdateRejectsInvalidStatus()
    {
        ...
        var result = controller.Update(created.Id, new UpdateTaskRequest(null, null, null, "done"));
        var badRequest = AssertIsType<ObjectResult>(result.Result, "Expected ObjectResult.");
        var problem = AssertIsType<ProblemDetails>(badRequest.Value, "Expected ProblemDetails.");
        ...
        AssertEqual(
            "Status must be pending, in_progress, completed, cancelled or archived.",
            problem.Detail,
            "Expected invalid status detail.");
        await Task.CompletedTask;
    }
    ...
    private static async Task ControllerDeleteReturnsNoContentForExistingTask()
    {
        ...
        AssertEqual("archived", service.Get(created.Id).Value!.Status, "Expected archived status.");
        await Task.CompletedTask;
    }
