...
public sealed class TasksApiTests : IClassFixture<TaskBoardWebApplicationFactory>
{
    ...
    {
        var response = await client.PostAsJsonAsync("/tasks", new CreateTaskRequest("  Check lab result  "));
    ...
    public async Task List_RejectsInvalidStatus_WithProblemDetails()
    {
        ...
        Assert.Equal("Status must be pending or completed.", problem.Detail);
    }
    ...
    public async Task Update_ReturnsNotFound_WhenTaskDoesNotExist()
    {
        var response = await client.PatchAsJsonAsync(
            $"/tasks/{Guid.NewGuid()}",
            new UpdateTaskRequest(null, "completed"));
    ...
    public async Task Update_RejectsEmptyPatchPayload()
    {
        ...
        var response = await client.PatchAsJsonAsync(
            $"/tasks/{created!.Id}",
            new UpdateTaskRequest(null, null));
    ...
    public async Task Update_RejectsInvalidStatus()
    {
        ...
        var response = await client.PatchAsJsonAsync(
            $"/tasks/{created!.Id}",
            new UpdateTaskRequest(null, "done"));
